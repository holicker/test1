...
import DetailMerchandiseMap from "../../../components/map/merchandise/DetailMerchandiseMap";
...
import { readMerchandise, unloadMerchandise } from "../../../modules/merchandise";

const DetailMerchandiseMapContainer = ({ match, history }) => {
  ...
  const { merchandise, error, loading } = useSelector(({ merchandise, loading, user }) => ({
  ...
  return (
    <DetailMerchandiseMap
      merchandise={merchandise}
      loading={loading}
      error={error}
      actionButtons={<MerchandiseActionButton onEdit={onEdit} onRemove={onRemove} />}
    />
  );
};

export default withRouter(DetailMerchandiseMapContainer);
